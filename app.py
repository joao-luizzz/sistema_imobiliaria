import streamlit as st
import calculos
import database
import relatorios

# Configuração Inicial
st.set_page_config(page_title="Imobiliária PG - MVP", layout="wide", page_icon="🏢")

def main():
    # Garante que o DB existe ao iniciar
    database.init_db()

    st.title("🌊 Sistema de Vendas - Praia Grande")
    st.markdown("### Simulador de Financiamento Direto")

    # --- SIDEBAR (Entradas) ---
    with st.sidebar:
        st.header("Dados do Cliente")
        cliente = st.text_input("Nome", "João Silva")
        renda = st.number_input("Renda Familiar (R$)", 1000.0, step=500.0)
        
        st.divider()
        st.header("Valores do Imóvel")
        valor_imovel = st.number_input("Valor Total (R$)", 100000.0, value=350000.0, step=1000.0)
        entrada = st.number_input("Entrada (R$)", 0.0, value=50000.0, step=1000.0)
        chaves = st.number_input("Chaves (R$)", 0.0, value=30000.0, step=1000.0)
        
        st.divider()
        st.header("Condições")
        meses = st.slider("Parcelas Mensais", 12, 120, 100)
        qtd_intercaladas = st.number_input("Qtd. Anuais", 0, 10, 8)
        valor_intercalada = st.number_input("Valor Anual (R$)", 0.0, value=5000.0, step=500.0)
        taxa_correcao = st.number_input("Taxa Simulação (% a.m.)", 0.0, 2.0, 0.5) / 100

    # --- PROCESSAMENTO (Usando o módulo calculos.py) ---
    saldo_devedor, total_intercaladas = calculos.calcular_saldo_devedor(
        valor_imovel, entrada, chaves, qtd_intercaladas, valor_intercalada
    )
    
    parcela = calculos.calcular_parcela(saldo_devedor, meses)
    
    cor_status, texto_status, msg_status, percentual_comp = calculos.analisar_credito(parcela, renda)

    # --- EXIBIÇÃO ---
    
    # 1. Alertas de Erro Matemático
    if saldo_devedor < 0:
        st.error("ERRO: A entrada + chaves + intercaladas supera o valor do imóvel!")
        return

    # 2. Métricas Principais
    col1, col2, col3 = st.columns(3)
    col1.metric("Parcela Mensal", f"R$ {parcela:,.2f}")
    col2.metric("Saldo a Financiar", f"R$ {saldo_devedor:,.2f}")
    col3.metric("Comprometimento", f"{percentual_comp:.1f}%")

    # 3. Status Visual (HTML/CSS)
    st.markdown(f"""
        <div style="background-color:{cor_status}; padding:15px; border-radius:10px; color:white; text-align:center;">
            <h3>{texto_status}</h3>
            <p>{msg_status}</p>
        </div>
    """, unsafe_allow_html=True)
    st.write("") # Espaçamento

    # 4. Gráfico de Evolução (Usando calculos.py)
    st.subheader("📊 Projeção de Parcela (Juros Compostos)")
    df_grafico = calculos.projetar_evolucao(saldo_devedor, parcela, meses, taxa_correcao)
    st.line_chart(df_grafico, x="Mês", y="Valor Parcela")

    # 5. Ações (Salvar e PDF)
    c1, c2 = st.columns(2)
    
    # Preparar dados para salvar/imprimir
    dados_completos = {
        'cliente': cliente,
        'valor_imovel': valor_imovel,
        'entrada': entrada,
        'chaves': chaves,
        'total_intercaladas': total_intercaladas,
        'saldo_devedor': saldo_devedor,
        'meses': meses,
        'parcela': parcela,
        'status_texto': texto_status
    }

    with c1:
        if st.button("💾 Salvar Simulação"):
            database.salvar_simulacao(cliente, valor_imovel, entrada, parcela, texto_status)
            st.success("Salvo com sucesso!")

    with c2:
        if st.button("📄 Gerar PDF"):
            arquivo = relatorios.gerar_proposta_pdf(dados_completos)
            with open(arquivo, "rb") as f:
                st.download_button("⬇️ Baixar PDF", f, file_name=arquivo)

    # 6. Histórico
    st.divider()
    st.subheader("Histórico Recente")
    st.dataframe(database.carregar_historico(), use_container_width=True)

if __name__ == "__main__":
    main()